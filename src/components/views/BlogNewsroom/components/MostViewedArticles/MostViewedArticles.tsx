...
const MostViewedArticles = ({ data, className, ...rest }: ViewComponentProps): JSX.Element => {
  ...
  const BlogContent = (props: any) => (
    <div className={classes.blogContent}>
      ...
      <LearnMoreLink title={"Learn more"} variant="body1" className={classes.button} />
    </div>
  )
    ...
    <div className={clsx(classes.root, className)} {...rest}>
      ...
      <Grid container spacing={0}>
        {data.map((item: any, index: number) => (
          <Grid item xs={12} key={index} data-aos="fade-up" className={classes.gridItem}>
            <CardProduct
